...
|---|---|---|
| `scripts/docker-start.sh` | `--with-chat`, `--build` | Starts Docker stack; waits for health |
| `scripts/docker-stop.sh` | `--with-chat` | Stops containers |
| `scripts/docker-restart.sh` | `--with-chat`, `--build` | Stop then start |
...
| `OB2_AUTO_ROUTE` | `false` | Enable classifier-based auto-routing (opt-in; the multi-domain path is the default for prefix-less queries). |
| `OB2_CLASSIFIER_MODEL` | *(same as OLLAMA_MODEL)* | Model for query classification (only used if `OB2_AUTO_ROUTE=true`). |
